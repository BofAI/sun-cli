...
describe('wallet', () => {
    ...
    it('rejects multiple wallet configuration modes', async () => {
        ...
        const { walletModule, resolveWalletProvider } = loadWalletModule();
        await expect(walletModule.initWallet()).rejects.toThrow('Set only one of TRON_PRIVATE_KEY, TRON_MNEMONIC, or AGENT_WALLET_PASSWORD.');
        expect(resolveWalletProvider).not.toHaveBeenCalled();
    });
    ...
    it('builds tronWeb with the active wallet as default address', async () => {
        ...
        expect(tronWeb.defaultAddress).toEqual({
            hex: '41abc',
    ...
    it('signs, broadcasts, and normalizes typed-data signatures', async () => {
        ...
        await walletModule.initWallet();
        const wallet = walletModule.getWallet();
        await expect(wallet.signAndBroadcast({ transaction: { raw_data: { contract: [] } } }, 'mainnet')).resolves.toEqual({ result: true, txid: 'tx-999' });
        ...
        await expect(wallet.signTypedData('Permit', { name: 'Sun', chainId: 1, verifyingContract: 'TContract' }, { Permit: [{ name: 'owner', type: 'address' }] }, { owner: 'TWalletAddress' })).resolves.toBe('deadbeef');
